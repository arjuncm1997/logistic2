...
from logistic.forms import Material, RegistrationForm, Purchaseview2,Purchaseview3,LoginForm, Offers,Account ,Changepassword,Reset,Changepassword, Materialedit,Reject,Paypal, Creditcard,Cart, Cart1,Purchaseview, Cartaddress, Cod
from logistic.models import Materials, Login, Offer,Credit, Pay, Contact, Cartmaterials, Timeoffer
...
@app.route('/')
def index():
    material = Materials.query.filter_by(status='approved',offer='no', timeoffer='no').all()
    material2 = Materials.query.filter_by(status='approved',offer='added').all()
    material3 = Materials.query.filter_by(status='approved',timeoffer='added').all()
    t = Timeoffer.query.all()
    return render_template("index.html",material=material,material2 = material2,  material3 = material3, t=t)
...
def uindex():
    ...
    material3 = Materials.query.filter_by(status='approved',timeoffer='added').all()
    t = Timeoffer.query.all()
    return render_template("uindex.html",material=material,material2 = material2, material3 = material3,t=t)
...
def login():
    ...
    if form.validate_on_submit():
        t=Timeoffer(name='asd')
        db.session.add(t)
        db.session.commit()
        user = Login.query.filter_by(email=form.email.data, usertype= 'admin').first()
...
def loginseller():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password,form.password.data):
            ...
            flash('Login Unsuccessful. Please check email and password', 'danger')
    return render_template("loginseller.html", form=form)
...
def s_addmaterials():
    ...
    if form.validate_on_submit():
        if form.pic.data:
            ...
            view = pic_file
        print(view)  
        gst = (int(form.price.data)*int(form.gst.data))/100
        netprice = int(form.price.data)+int(gst)
        material = Materials(sowner= current_user.username,sownerid= current_user.id,name=form.name.data,brand=form.brand.data,netprice=netprice,gstprice=gst,gst = form.gst.data,desc = form.desc.data, place=form.place.data,price = form.price.data,avail=form.avail.data,image=view, status='...')
...
@login_required
def smaterialsview():
    material = Materials.query.filter_by(sownerid=current_user.id)
...
def smaterialsedit(id):
    ...
    if form.validate_on_submit():
        ...
        material.gst = form.gst.data
        material.gstprice = (int(form.price.data)*int(form.gst.data))/100
        material.netprice = int(form.price.data)+int(material.gstprice)
        db.session.commit()
...
def amaterialsedit(id):
    ...
    if form.validate_on_submit():
        ...
        form.gst.data = material.gst
        form.gstprice.data = material.gstprice
        form.netprice.data = material.netprice
...
def ucart():
    form = Cart1()
    material = Cartmaterials.query.filter_by(cart = 'added',uownerid = current_user.id,).all()
...
def ucartto(id):
    form = Cart1()
    material = Cartmaterials.query.get_or_404(id)
    if form.validate_on_submit():
...
@login_required
def ucartremove(id):
    remove = Cartmaterials.query.get_or_404(id)
    try:
...
def ucartadd(id):
    ...
    material = Cartmaterials(uowner = current_user.username,uownerid = current_user.id,cart = 'added' ,name= add.name,sowner = add.sowner,brand = add.brand,avail = add.avail,price =  add.offerprice,place = add.place,image = add.image,desc = add.desc)
    db.session.add(material)
    db.session.commit()
...
def ucartadd1(id):
    add = Materials.query.get_or_404(id)
    material = Cartmaterials(uowner = current_user.username,uownerid = current_user.id,cart = 'added' ,name= add.name,sowner = add.sowner,brand = add.brand,avail = add.avail,price =  add.netprice,place = add.place,image = add.image,desc = add.desc)
    db.session.add(material)
    db.session.commit()
    flash('added to your cart')
    return redirect('/uindex')
...
def apurchasematerial():
    form =Purchaseview()
    material = Cartmaterials.query.filter_by(purchase = 'purchased').all()
...
@login_required
def umyorder():
    material = Cartmaterials.query.filter_by(uowner = current_user.username,purchase='purchased').all()
...
def aoffer():
    form = Purchaseview()
    material = Materials.query.filter_by( status= 'approved',timeoffer='no' ).all()
    return render_template("aoffer.html",material = material,form=form)

@app.route('/atimeroffer')
@login_required
def atimeroffer():
    form = Purchaseview2()
    material = Materials.query.filter_by( status= 'approved', offer='no').all()
    return render_template("atimeroffer.html",material = material,form=form)

@app.route('/atimerofferview')
@login_required
def atimerofferview():
    material = Materials.query.filter_by(timeoffer='added').all()
    return render_template("atimerofferview.html",material=material)

@app.route('/timeofferremove/<int:id>')
def timeofferremove(id):
    remove = Materials.query.get_or_404(id)
    remove.timeoffer = 'no'
    remove.offerprice = 'NULL'
    remove.discount = 'NULL'
    db.session.commit()
    return redirect("/atimerofferview")

@app.route('/atimerofferadd/<int:id>',methods = ['GET','POST'])
@login_required
def atimerofferadd(id):
    form = Purchaseview2()
    material = Materials.query.get_or_404(id)
    if form.validate_on_submit:
        material.offerprice = form.name.data
        material.timeoffer = 'added'
        dis= int(material.netprice)- int(material.offerprice)
        material.discount = dis
        db.session.commit()
        flash ("offer added")
        return redirect('/atimeroffer')
    elif request.method == 'GET':
...
def atime():
    form = Purchaseview3()
    t = Timeoffer.query.all()
    return render_template("atime.html", t=t,form =form)

@app.route('/anewtime/<int:id>',methods = ['GET','POST'])
@login_required
def anewtime(id):
    form = Purchaseview2()
    t = Timeoffer.query.get_or_404(id)
    if form.validate_on_submit:
        t.name = form.name.data
        t.date = form.date.data
        db.session.commit()
        flash ("Time added")
        return redirect('/atime')
    elif request.method == 'GET':
        form.name.data = t.name
        form.date.data = t.date
    return render_template('/atime.html',form=form,t = t)
